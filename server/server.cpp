...
void Server::onNewConnection()
{
    ...
    qDebug() << QString("a new connection from %1:%2 and connectionPool's size is %3")
                    .arg(connectToClient->peerAddress().toString())
                    .arg(connectToClient->peerPort())
                    .arg(connectionPool.size());
}
...
void Server::registerRequestHandle(QWebSocket *sender, QStringList msgList)
{
    ...
    QString id = query.last() ? QString::number(query.value(0).toInt() + 1) : "1";

    query.exec(QString("insert into acount values('%1','%2','%3')")
                   .arg(id)
                   .arg(name)
                   .arg(password)); //插入用户相关信息到用户表中
...
void Server::myInfoRequestHandle(QWebSocket *sender, QStringList msgList)
{
    ...
    //查询自己id对应的id和昵称作为自己的用户信息返回
    query.exec(QString("select name from acount where id='%1';").arg(id));
    //整理发送的返回消息
...
void Server::friendListRequestHandle(QWebSocket *sender, QStringList msgList)
{
    ...
    //查询相应id的好友表中的所有的好友的id和昵称
    query.exec(QString("select id,name from acount,friend%1 where acount.id=friend%2.friendid;").arg(id).arg(id));
    //整理发送返回的消息
...
void Server::messageHandle(QWebSocket *sender, QStringList msgList)
{
    ...
    sender->sendTextMessage(QString("Message %1 %2 %3")
                                .arg(recvID)
                                .arg(msgList[1])
                                .arg(tr(u8"该用户目前没有上线，你的消息可能无法到达！")));
}
