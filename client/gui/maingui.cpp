...
void MainGui::friendListItemDoubleClicked(QListWidgetItem *item)
{
    ...
    //没有结束的话，那肯定是没有打开，则打开
    ChatFrame *chatFrame =
        new ChatFrame(framePlace, UserInfo(item->data(Qt::UserRole + FriendInfoNum::id).toString(),
                                           item->data(Qt::UserRole + FriendInfoNum::name).toString()));
    setWindowTitle(windowTitle().split("---")[0] + "---" + item->data(Qt::UserRole + FriendInfoNum::id).toString() +
                   "-" + item->data(Qt::UserRole + FriendInfoNum::name).toString());
...
void MainGui::updateFriendList(QStringList friendList)
{
    ...
    for (int i = 0; i < chatFramePool.count(); i++)
    {
        ...
        {
            if (this->friendList->item(j)->data(Qt::UserRole + FriendInfoNum::id).toString() ==
                chatFramePool[i]->getID())
            {
                //如果存在
                isExist = true;
                break;
...
void MainGui::addFriendActionTriggered()
{
    ...
    if (!isAddFriendGuiOpen)
    {
        ...
        connect(this, &MainGui::noThisUserSignal, addFriendGui,
                &AddFriendGui::noThisUserSlot); //添加好友的时候返回没有这个用户
        addFriendGui->show();
    }
}
...
void MainGui::delFriendActionTriggered()
{
    ...
    if (items.count())
        //判断是否要删除好友
        if (QMessageBox::warning(this, tr(u8"警告"),
                                 tr((QString(u8"你真的要删除你的好友 %1 吗？")
                                         .arg(items[0]->data(Qt::UserRole + FriendInfoNum::name).toString())
                                         .toUtf8())),
